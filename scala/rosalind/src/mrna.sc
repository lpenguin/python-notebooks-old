...
val (a, b, c, d) = (29, 73, 10, 31)
val n = 11
(a * b * c * d) % n
(((((a * b) % n) * c) % n) * d) %n
val codonTableStr:String = """UUU F      CUU L      AUU I      GUU V
...
"""
val codonTable = new scala.collection.mutable.HashMap[String, Int]()
val tuples = codonTableStr.split("\\s+")
.sliding(2, 2)
.foreach((t:Array[String]) => {
  val Array(value, key) = t
  val count = codonTable.getOrElse(key, 0)
  codonTable(key) = count + 1
})

println(codonTable.values.sum)
